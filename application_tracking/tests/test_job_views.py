...
def test_search(client: Client):
    ...
    url = reverse("search") + "?keyword=Python&location=Kathmandu"
    response = client.get(url)

    assert response.status_code == 200
    job_adverts_page = response.context["job_adverts"]
    job_adverts = job_adverts_page.object_list

    assert job1 in job_adverts
    assert job2 not in job_adverts
    assert job3 not in job_adverts
    assert job4 not in job_adverts